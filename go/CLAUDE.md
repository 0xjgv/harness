...
- Suppressions: `go run harness.go suppressions` — full suppression breakdown; `--update-baseline` requires human sign-off and rewrites `.harness-baseline`. It merges: every key it measures (`suppressions.*`, `coverage.min`, `complexity.max_violations`, `crap.max_violations`, `arch.max_violations` — the last of which runs go-arch-lint, so it fetches a module on a cold cache) is overwritten — a kind that vanished is recorded as `0` so the floor ratchets down — and every key it does not own (`mutation.min`, anything hand-written) is carried through untouched. It is all-or-nothing: a metric that does not apply here has its key dropped with a `⚠`, and a metric whose tool ran and failed aborts the write entirely, because a floor recorded from a broken run is worse than no floor.
- Arch: `go run harness.go arch` — go-arch-lint@v1.18.0 against `.go-arch-lint.yml`, counted from its `--json` report (dependency, not-matched and deep-scan notices, plus any the tool omitted) and compared against `arch.max_violations` in `.harness-baseline`; at or below that floor the gate passes and prints the count. The tool's own exit code is ignored — it exits 1 whenever it reports anything, which is not the same as being over the floor. With no such key — or no `.harness-baseline` at all — the gate is report-only: it prints `report-only: no .harness-baseline floor` plus the current count and passes, so a repo adopting the harness with a boundary already crossed is green on day one. Record a floor with `suppressions --update-baseline`. Skipped with a warning when `.go-arch-lint.yml` is absent. A run that produces no JSON, or one that reports execution warnings (a component pointing at a directory that no longer exists, a malformed rule — go-arch-lint then skips the boundary check and reports nothing), is a failure, never a clean tree: it also aborts `suppressions --update-baseline` rather than recording a floor of 0.
