...
- Mutation (advisory): `go run harness.go mutation` — gremlins kill-rate, scoped to the package directories of the changed `.go` files (`git diff <base>...HEAD` when a base ref resolves, otherwise the uncommitted set; `--base=<ref>` picks the base, `--all` or an explicit path argument mutates the concrete targets instead). Packages with no `*_test.go` are skipped with a `⚠`, and an empty scope skips the gate — a scoped gate never widens to the whole tree. The score is `killed / (killed + survived)` rounded, compared against `mutation.min` in `.harness-baseline`; with no such key the gate is report-only and passes even under `--enforce`. `--enforce` exits 1 below a recorded floor (default warns and exits 0). `--report=<path>` scores a report gremlins already wrote instead of spending minutes on a fresh run; an unreadable path exits 1. Scoping is package-granular because gremlins' own `--diff` silently matches nothing when it is pointed at a concrete package. Caveat worth knowing: `mutation.min` is measured over the concrete targets, so a scoped run on a weaker-than-average package can warn — which is why the gate is advisory.
