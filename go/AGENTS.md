...
- After edits: `go run harness.go check` — fix, format, lint (via `--fix`), test (scoped — see below), complexity, `go mod tidy -diff`, acceptance (self-skips with a warning when no `.feature` files exist), arch config guard (warn), Gherkin-first guard (warn), agents-md drift, suppression ratchet. **Invariant: `check` runs every gate that is offline, fast, and takes no build lock.** `ci` adds: the dependency audit (network, govulncheck), coverage, CRAP (advisory) — and, uniquely in this template, the `arch` gate: `go run github.com/fe3dback/go-arch-lint@…` fetches a module, which hits the network on a cold module cache, so it stays `ci`/`pre-push`-only rather than running in the edit-triggered `check` loop.
- Pre-commit: `go run harness.go pre-commit` — staged files only (auto via git hook). Fix/format rewrite the working tree, then the fixed files are re-staged (`git add`) so the commit records the fixed content, then the staged packages' tests run (staging only `_test.go` files now runs that package instead of skipping tests). Caveat: if a file is only partially staged, `git add` also stages its unstaged hunks.
- Scoped tests: `check` and `pre-commit` run `go test` only over the packages the change touches — one `./<dir>/...` per changed `.go` file's directory, deduped (`harness.go` maps to no package: it carries `//go:build ignore`). An empty scope prints a `⚠` and skips; it never widens to the whole tree. A changed package with no `*_test.go` gets one `⚠` naming it and never fails — the packages that do have tests still run. `--all` runs the whole suite instead. In `check` (not `pre-commit`, which is staged-only), `--base=<ref>` — or `HARNESS_ARCH_BASE` / `GITHUB_BASE_REF` — adds `git diff <ref>...HEAD` to the uncommitted set, and warns rather than passing silently when that ref does not resolve. `ci` runs the whole suite under coverage; `pre-push` runs no tests at all (it is the lint/acceptance/arch gate).
