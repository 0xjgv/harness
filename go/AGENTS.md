...
- CI: `go run harness.go ci` — read-only gates (lint, audit, complexity, duplication, `go mod tidy -diff`, acceptance, arch) run in parallel — captured, printed in submission order, run to completion — then test-cov (streams) + crap, then arch config guard and Gherkin-first guard, both blocking. CRAP is advisory (warns only — pass `--enforce` to hard-fail). Requires `uvx` on PATH.
...
- Duplication: `go run harness.go complexity` runs it too — a second lizard invocation (`-Eduplicate`) over the same target set and exclusions, counting the copy-pasted blocks it reports (70+ unified tokens each) against `duplication.max_blocks` in `.harness-baseline`. Same rule as complexity: with no such key the gate is report-only and passes. Lizard's exit code only tracks CCN warnings, so the runner does the comparing. Runs wherever the complexity gate runs (`check`, `ci`, `stop-hook`).
