...
uv run harness stop-hook             # post-edit, then lint/complexity/dead code on the change; silent on success, exit 2 with findings (agent Stop hook)
...
files. It blocks (exit 2, findings on stderr, at most 20 lines) on lint and vulture
findings on changed lines, and on any function in `src/` or `tests/` that is over a
lizard limit and overlaps a changed line. Touching an over-limit function means
leaving it under the limits; untouched debt never blocks. It prints nothing on
success. Exit 1 means a tool could not run, or the hook already blocked this stop
(`stop_hook_active`). An uncommitted `CLAUDE.md` edit is copied to `AGENTS.md`.
