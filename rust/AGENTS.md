...
- Tests: `cargo harness test` — whole suite. `check`/`pre-commit` scope it to the tests that map to changed files: `src/lib.rs` → `--lib`, `src/foo/bar.rs` → libtest filter `foo::bar`, `src/foo/mod.rs` → `foo`, `src/main.rs` and `src/bin/*.rs` → `--bins`, `tests/<name>.rs` → `--test <name>`, any `.feature` → `--test acceptance`, `harness.rs` → `--bins` + `--test acceptance`. Filters never run bare (`cargo test -- <filter>` would forward the filter to the `harness = false` acceptance target): they run under `--lib`, or `--bins` in a crate with no lib target, OR'd in one invocation. Change set: the staged files in `pre-commit`, else `<base>...HEAD` when `--base=<ref>` / `HARNESS_ARCH_BASE` / `GITHUB_BASE_REF` resolves, else the uncommitted files. Nothing changed → warn and skip, never widen; a changed source that maps to no test (no `#[cfg(test)]` block) → one warn line, never a failure; a `--base=<ref>` git cannot resolve → the gate fails rather than falling back to a scope that would test nothing. `--all` and `ci` run the whole suite. Assumes an integration target is named after its file stem and the cucumber target is called `acceptance`.
...
- Acceptance: `cargo harness acceptance` — run cucumber against `tests/features/`. Not a separate gate in `check`: the `acceptance` test is a Cargo `[[test]]` target (`tests/acceptance.rs`, `harness = false`), so `cargo test` executes it as part of `check`'s "Tests" step whenever the scope maps to it (a changed `.feature` or `harness.rs`, or `--all`) — unlike python/bun/go, whose acceptance runner is a standalone script their default test command does not invoke automatically, so those templates need `check` to call it separately.
