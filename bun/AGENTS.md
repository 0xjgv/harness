...
- Ratcheted floors (`complexity.max_violations`, `crap.max_violations`, `arch.max_violations`, `coverage.min`, `suppressions.*`) only move down: regenerate with `bun run suppressions --update-baseline`, never hand-raise a number.
