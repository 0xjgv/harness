...
- After edits: `bun run check` — lockfile sync (`bun install --frozen-lockfile`), fix, format, typecheck, test (warns/skips when no tests exist), complexity, duplication, deadcode, acceptance, arch (all run as a read-only parallel batch after the fix step), hook-drift + arch-config-guard (warn) + gherkin-guard (warn) + suppression ratchet
...
- CI: `bun run ci` — read-only gates (lint, typecheck, audit, complexity, duplication, deadcode, acceptance, arch) run in parallel — captured, printed in submission order, run to completion — then coverage (streams) + crap. CRAP is advisory (warns only — pass `--enforce` to hard-fail). Requires `uvx` on PATH.
...
- Stop hook: `bun run stop-hook` — auto-formats/fixes changed files, warns via `arch-config-guard` and `gherkin-guard`, then runs complexity, duplication and deadcode in parallel. On failure it exits 2 and writes a short summary to stderr (which Claude Code's Stop hook treats as blocking and feeds back to the model); Codex's wrapper (`.codex/hooks/codex-stop-hook.sh`) turns any non-zero exit into a block regardless.
...
- Ratcheted floors (`complexity.max_violations`, `duplication.max_blocks`, `crap.max_violations`, `coverage.min`, `suppressions.*`) only move down: regenerate with `bun run suppressions --update-baseline`, never hand-raise a number.
