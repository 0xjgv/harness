...
- CRAP (advisory): `bun run crap --max=30` — complexity × coverage gate. Offenders above `--max` are compared to the `crap.max_violations` floor in `.harness-baseline` (no floor = 0 tolerance, safe because the gate is advisory): ✓ while the count stays at or below the floor, warn above it. Add `--enforce` to exit 1 above the floor (default exits 0 with warning). Warns and skips when no tests or coverage artifact exist.
