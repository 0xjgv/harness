...
- CI: `bun run ci` — read-only gates (lint, typecheck, audit, complexity, deadcode, acceptance, arch) run in parallel — captured, printed in submission order, run to completion — then coverage (streams) + crap + mutation (scoped to the base-ref diff). CRAP and mutation are advisory (warn only — pass `--enforce` to hard-fail). Requires `uvx` on PATH.
...
- Ratcheted floors (`complexity.max_violations`, `crap.max_violations`, `coverage.min`, `mutation.min`, `suppressions.*`) only move down: regenerate with `bun run suppressions --update-baseline` (add `--with-mutation` to re-measure `mutation.min`), never hand-raise a number.
