...
- [ ] Include a **stop-hook** command that runs post-edit, then blocks only on the change: lint on changed lines, complexity new or worse than the merge-base (+ deadcode on changed lines where shipped). Silent on success, exit 2 with findings on stderr, exit 1 on tool failure, loop guard on `stop_hook_active` (see `skills/harness/reference/settings-json.md`)
